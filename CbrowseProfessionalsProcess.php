...
if (isset($_GET['filter'])) {
    ...
    if (is_array($filter) && isset($filter['areaOfWork'])) {
        ...
        $sql1 = "SELECT p.*, pd.areaOfWork as areaOfWork
                FROM professional p
...
FROM professional p
JOIN professional_details pd ON p.id = pd.professionalID
JOIN cont_pro_feedback C ON p.id = C.professionalID
...
if (!empty($sql1)) {
                    ...
                    while ($row1 = $result1->fetch_assoc()) { ?>
                        ...
                        <td><?php echo $counter+=1; ?></td>
                         ...
                         <td><?php echo "<b>Phone number: </b>". ucwords($row1['phoneNumber'])."<br><b>Email: </b>".ucwords($row1['email']); ?></td>
                            <td>
                                <a href="#" class="btn btn-outline-secondary view-feedback-btn mb-1"
                                   data-professional-id="<?php echo $row1['id']; ?>"
                                   data-bs-toggle="modal" data-bs-target="#feedbackModal">Check Feedback</a>
                                <a href="#" class="btn btn-outline-secondary view-cv-btn"
                                   data-professional-id="<?php echo $row1['id']; ?>"
                                   data-bs-toggle="modal" data-bs-target="#cvModal">View CV</a>
                            </td>
                        </tr>
                    <?php } ?>
                </tbody>
    ...
    echo '<div class="alert alert-danger text-center" role="alert">No professionals found for the selected area.</div>';
}
      ...
      <table class="table table-striped table-bordered text-center align-middle">
          ...
          <?php 
           ...
           while ($row2 = $result2->fetch_assoc()) { ?>
              <tr>
              <td><?php echo $counter+=1; ?></td>
                  <td><?php echo htmlspecialchars($row2['fullName']); ?></td>
                  <td><?php echo htmlspecialchars($row2['areaOfWork']); ?></td>
                         <td><?php echo "<b>Phone number: </b>". ucwords($row2['phoneNumber'])."<br><b>Email: </b>".ucwords($row2['email']); ?></td>
                 <td style="color: red; font-weight: bold;"><?php echo htmlspecialchars($row2['avg_rating']); ?></td>

                  <td>
                  <a href="#" class="btn btn-outline-secondary view-cv-btn" data-professional-id="<?php echo $row2['id']; ?>" data-bs-toggle="modal" data-bs-target="#cvModal">View CV</a>
                  </td>
