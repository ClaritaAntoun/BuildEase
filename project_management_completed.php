...
// Calculate total cost (actual materials)
$total_cost = $actual_materials_cost;
...
// Handle material feedback update
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_feedback'])) {
    $material_id = intval($_POST['material_id']);
    $feedback = $_POST['feedback'];
    
    $update_sql = "UPDATE work_materials SET feedback = ? 
                  WHERE id = ? AND professionalID = ? AND projectID = ? AND stepNumber = ?";
    $update_stmt = $conn->prepare($update_sql);
    $update_stmt->bind_param("siiii", $feedback, $material_id, 
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editMaterialModalLabel">Edit Material Feedback</h5>
